function data_iterator = iterator_isi_framelet_exp( CellNum , RunNum , N_isi , SpikeHistoryCells , powers , type , skip)
% SIGNATURE:
...
load('~/.kolia/matlab_prefix')

% load /Users/ksadeghi/Documents/projects/framelet_strips/Data2/wavelet2/data/refcell_old
load(sprintf('%s/kolia/ENCODING/DATA/strips/GoodCells/GoodCells_JustINDS',matlab_prefix))
...
if length(N_isi) < 1
    ...
    SpikeHistoryExtractor = @(all_spike_times , times) exps( powers , ISIextractor(all_spike_times , times , N_isi)) ;
...
cel = load(sprintf('%s/kolia/ENCODING/DATA/strips/wavelet_transformed/spike_times/Data_Cell_%d_Run_%d.mat',matlab_prefix,CellNum,RunNum)) ;
...
%     load(sprintf('%s/kolia/ENCODING/DATA/strips/wavelet_transformed/reference_times/Metronome_refcell_%d_Run_%d.mat',matlab_prefix, CellNum,RunNum)) ;
    chosen_data_index = find(mod(1:N,80+20+12+skip)<=80) ;
elseif strcmp(type , 'validate')
    refdata = [] ; reftimes = [] ;
    for rrr=1:6
        if rrr ~= RunNum
            load(sprintf('%s/kolia/ENCODING/DATA/strips/wavelet_transformed/reference_times/Metronome2_refcell_%d_Run_%d.mat',matlab_prefix,CellNum,rrr)) ;
            refdata = [refdata ; refcell.data] ;
            reftimes = [reftimes refcell.times] ;
        end
    end
    refcell.data = refdata ;
    refcell.times = sort(reftimes) ;
%     load(sprintf('%s/kolia/ENCODING/DATA/strips/wavelet_transformed/reference_times/Metronome2_refcell_%d_Run_%d.mat',matlab_prefix, CellNum,RunNum)) ;
...
keep_temp = abs(sum(reference_histories,2)) ;
keepers = find(isfinite(keep_temp) & keep_temp<1e3) ;
data_iterator.reference.data = [data_iterator.reference.data(keepers,:)  reference_histories(keepers,:)] ;
clear refcell

spike_histories = SpikeHistoryExtractor(all_spike_times,spike_times) ;
keep_temp = abs(sum(spike_histories,2)) ;
keepers = find(isfinite(keep_temp) & keep_temp<10) ;
data_iterator.spikes.data = [data(keepers,:) spike_histories(keepers,:)] ;

% fprintf('\nSpike History size')
% size(spike_histories)

data_iterator.info.SpikeHistoryCells = SpikeHistoryCells ;
data_iterator.info.powers = powers ;
data_iterator.info.N_cells = N_cells ;
data_iterator.info.N_isi = N_isi ;
...
data_iterator.spikes.N = size(data_iterator.spikes.data,1) ;
